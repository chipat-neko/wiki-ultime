...
import {
  ...
  FileText, Star, RefreshCw, ChevronDown, ChevronUp, Eye,
} from 'lucide-react';
...
export default function AdminPanel() {
  ...
  async function handleApprove(userId) {
    ...
    try { await approveUser(userId); await reload(); }
  ...
  async function handleReject(userId) {
    ...
    setActionLoading(true);
    try { await rejectUser(userId); await reload(); }
    catch (e) { alert('Erreur : ' + e.message); }
    finally { setActionLoading(false); }
  }
  ...
  async function handleAcceptContrib(id) {
    setActionLoading(true);
    try { await acceptContribution(id, starsInput); await reload(); setExpandedContrib(null); }
    catch (e) { alert('Erreur : ' + e.message); }
    finally { setActionLoading(false); }
  }

  async function handleRejectContrib(id) {
    setActionLoading(true);
    try { await rejectContribution(id, rejectNote); await reload(); setExpandedContrib(null); setRejectNote(''); }
    catch (e) { alert('Erreur : ' + e.message); }
    finally { setActionLoading(false); }
  }
                    ...
                    <div className="border-t border-space-400/20 p-4 space-y-4">
                      ...
                      {c.status === 'pending' && (
                        <div className="flex items-end gap-3 pt-2 border-t border-space-400/10">
                          ...
                          <button
                            onClick={() => handleRejectContrib(c.id)}
                            disabled={actionLoading}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 text-sm transition-colors"
                          >
                            <XCircle className="w-4 h-4" />
